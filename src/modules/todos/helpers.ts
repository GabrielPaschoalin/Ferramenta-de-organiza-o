import type { Category, CategoryFilter, StatusFilter, Task } from '@/modules/todos/types'
...
export function filterTasks(
  ...
  return tasks
    .filter((task) => {
      if (status === 'concluidas') return task.done
      if (status === 'hoje') return task.dueDate === today
      return true
    })
    .filter((task) => {
      if (category === 'all') return true
      if (category === 'none') return !task.categoryId
